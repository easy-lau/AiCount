...
- **项目页**：所有项目卡片墙，搜索 + 排序（最近活跃 / 名称 / 会话数），点击进入概况页
- **会话浏览**：列出所有解析到的 JSONL 会话，按 provider/项目/日期过滤，点击查看事件时间线与「在 Finder 中显示」
- **设置**：自定义 Claude / Codex 数据目录、扫描排除规则、主题（跟随系统/浅色/深色）、托盘显示项、清理缓存
